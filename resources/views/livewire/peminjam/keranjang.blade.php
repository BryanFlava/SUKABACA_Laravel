<div class="container">
    ...
    <div class="row">
        <div class="col-md-12">
                ...
                <tbody>
                    @foreach ($keranjang->detail_peminjaman as $item)
                        <tr>
                            ...
                            <td>
                                @if ($keranjang->tanggal_pinjam)
                                <button type="button" wire:click="hapus({{$keranjang->id}}, {{$item->id}})" class="btn btn-sm btn-danger">Hapus</button>
                                @endif       
                            </td>
                        </tr>
                    @endforeach
            ...
            @if ($keranjang->tanggal_pinjam)
                 <button type="button" wire:click="hapusMasal" class="btn btn-danger">Hapus Masal</button>
            @endif  
            
            <button type="button" wire:click="pinjam({{$keranjang->id}})" class="btn btn-success" >Pinjam</button>
        </div>
    </div>
</div>